import { z } from "zod"
import { BUY_BACK_TYPES, DEVICE_TYPES, PAYMENT_METHODS, PRODUCT_CATEGORIES } from "@/lib/constants"

export const loginSchema = z.object({ email: z.email().transform((v) => v.toLowerCase()), password: z.string().min(8) })
export const registerSchema = loginSchema.extend({ name: z.string().min(2).max(80), phone: z.string().min(7).max(20), idNumber: z.string().min(3).max(30) })
...
export const adminCreateCustomerSchema = registerSchema.extend({ address: z.string().max(300).optional().or(z.literal("")) })
// Admin-initiated repair intake: contact is optional since it can fall back to
// the selected customer's phone number on file.
export const adminRepairSchema = z.object({ customerId: z.string().min(1), deviceType: z.enum(DEVICE_TYPES), brand: z.string().min(2), model: z.string().min(1), problem: z.string().min(10), contact: z.string().min(7).optional().or(z.literal("")), image: z.url().optional().or(z.literal("")) })
...
export const adminBuyBackSchema = z.object({ customerId: z.string().min(1), deviceType: z.enum(BUY_BACK_TYPES), brand: z.string().min(2), model: z.string().min(1), specifications: z.string().min(5), condition: z.string().min(2), description: z.string().min(10), expectedPrice: z.coerce.number().positive(), offeredPrice: z.preprocess((v) => (v === "" ? undefined : v), z.coerce.number().positive().optional()), image: z.url().optional().or(z.literal("")) })
export const posSaleSchema = z.object({ customerId: z.string().min(1), paymentMethod: z.enum(PAYMENT_METHODS), paymentNote: z.string().max(200).optional().or(z.literal("")), items: z.array(z.object({ productId: z.string().min(1), quantity: z.coerce.number().int().positive() })).min(1) })
